...
public class DataServicesDao implements IDataServicesDao {
	...
	public Article getDataByLink(LinkBody linkBody) {
		...
		case "shein":
			...
			break;
		case "eastpark":
			article = getDataEastPark(linkBody);
			break;
		default:
			break;
		}
	...
	/**
	 * Méthode pour récuperer le nom, le prix, et l'image sur la page de détail de
	 * la page amazon
	 * 
	 * @author katina
	 * 
...
//		Ouvrir la page web
		...
		name = driver.findElement(By.xpath("//*[@id=\"goods-detail-v3\"]/div/div[1]/div/div[2]/div[2]/div/div[1]/h1"))
				.getText();
	 ...
	 * @return Article
	 */
	private Article getDataEastPark(LinkBody linkBody) {
		String name;
		WebElement price1;
		double price;
		WebElement imagePath;
...
//		Ouvrir la page web
		...
		name = driver.findElement(By.xpath("//*[@id=\"maincontent\"]/div/div/div[3]/div/div[2]/h1/span"))
				.getText();
		price1 = driver.findElement(
				By.className("price"));
		price = Double.parseDouble(removefirstChar(price1.getAttribute("innerHTML"), 7).replace(",", "."));
		imagePath = driver.findElement(By.xpath(
				"//*[@id=\"maincontent\"]/div/div/div[2]/div[2]/ul/li[1]/img"));
		image = imagePath.getAttribute("src");
		Article article = new Article(1, name, price, image, linkBody.getOrigin());
		driver.quit();
		return article;
	...
	public static String removefirstChar(String str, int n) {
		if (str == null || str.length() == 0) {
			return str;
		}
